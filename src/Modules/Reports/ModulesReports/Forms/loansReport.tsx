...
const LoanReport = ({ handleClose, reportType }: Props) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<NewEquipmentTypeInputs> = async (data) => {
    if (reportType == 1) {
      const result = await window.Main.generateLoanEquipment(
        data.StartDate,
        data.EndDate
      );
      if (result === 1) {
      ...
      const result = await window.Main.generateLoanMaintenanceEquipment(
        data.StartDate,
        data.EndDate
      );
      if (result === 1) {
